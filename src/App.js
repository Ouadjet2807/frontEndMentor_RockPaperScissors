...
function App() {
  ...
  const fighters = [
    ...
    },
    {
      ...
      name: "Spock",
      beats: ["Scissors", "Rock"],
      icon: "Assets/images/icon-spock.svg"
    },
    {
      id: 3,
      name: "Paper",
    ...
    },
    {
      ...
      name: "Lizard",
      beats: ["Paper", "Spock"],
      icon: "Assets/images/icon-lizard.svg"
    },
    {
      id: 5,
      name: "Rock",
  ...
  const selectWinner = () => {
    let newScore
    if(houseChoice.beats.includes(userChoice.name)) {
      if(score > 0) {
        newScore = score -1
        setResult("loose")
        setEndGameMessage('You lose')
      }
    } else if(userChoice.beats.includes(houseChoice.name)) {
      newScore = score + 1
      setResult("win")
      setEndGameMessage('You win')
    } else if(houseChoice.name === userChoice.name) {
      setResult("null")
      newScore = score
  ...
  useEffect(() => {
    ...
    if(storage !== undefined && storage !== null) {
      let storedValue = parseInt(storage)
  ...
  return (
    <div className="App">
      ...
      <div className={`game ${userChoice !== undefined ? "started" : ""}`}>
      
        <div className="pentagon" >
          {userChoice !== undefined &&
            <div className="container">
             <div className={`choosed ${gameState}`}>
              <div className="userChoice">
                <h3>You picked</h3>
                <div className="choiceBg" id={userChoice.name.toLowerCase()}>
                  <div className="choice">
                    <img src={userChoice.icon} alt=""/>
                  </div>
                </div>
              </div>
              {gameState === "complete" &&
              ...
              <div className="houseChoice" id={`${houseChoice !== undefined ? "defined" : 'undefined'}`}>
                <h3>The house picked</h3>
                <div className="choiceBg" id={houseChoice && houseChoice.name.toLowerCase()}>
                  <div className="choice">
                    {houseChoice !== undefined ? 
                    <img src={houseChoice.icon} alt=""/>
                    :
          ...
          <div className="choices">
            ...
            {fighters.map(item => (
              <div  key={`move_${item.name}`} className={`choiceBg ${userChoice && userChoice.name === item.name ? "choosedMove" : ""}`} id={item.name.toLowerCase()} onClick={() => setUserChoice(item)}>
              <div className="choice">
          ...
          <svg xmlns="http://www.w3.org/2000/svg" className="bg" viewBox="0 0 330 330" preserveAspectRatio="xMidYMid"><path style={{zIndex: `${gameState === "complete" ? '0' : '1'}`}} fill="none" stroke="#000" strokeWidth={width > 375 ? "15" : "25"} width="100%" height="100%" d="M164.5 9.27L9.26 122.06l59.296 182.495h191.888L319.74 122.06 164.5 9.271z" opacity=".2"/></svg>
        </div>
      
     
      </div>
      <div className="rules">
        <button onClick={() => setOpenRules(true)}><span>rules</span></button>
      </div>
    </div>
  );
}
